...
def remove_local_path(path: str):
    ...
    # First try: Force close any Git processes that might be using the directory
    try:
    ...
    # Second try: Use robocopy to clear readonly attributes (Windows-specific)
    if os.name == 'nt':
        try:
            subprocess.run(['attrib', '-r', '/s', '/d', path + '\\*.*'], 
                         capture_output=True, check=False)
        except Exception:
            pass
    
    # Third try: Standard removal with retry logic
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"[CLEANUP] Attempt {attempt + 1} failed, retrying in 2 seconds: {e}")
                time.sleep(2)
            else:
                logger.error(f"[CLEANUP] Failed to remove {path} after {max_retries} attempts: {e}")
                # Last resort: rename the directory and let Windows cleanup later
                try:
                    import uuid
                    temp_name = f"{path}_delete_{uuid.uuid4().hex[:8]}"
                    os.rename(path, temp_name)
                    logger.info(f"[CLEANUP] Renamed {path} to {temp_name} for later cleanup")
                except Exception as rename_exc:
                    logger.error(f"[CLEANUP] Even rename failed: {rename_exc}")
                    raise e
...
async def generate_files_and_deploy(task_data: TaskRequest):
    ...
    try:
        ...
        # Close the Git repository properly to release file handles
        try:
            repo.close()
            del repo  # Remove reference to help with cleanup
        except Exception as close_exc:
            logger.warning(f"Failed to close repo properly: {close_exc}")
        ...
        # Try to close repo if it exists and cleanup
        try:
            if 'repo' in locals():
                repo.close()
                del repo
        ...
        # Final cleanup of local directory
        try:
            if 'local_path' in locals() and os.path.exists(local_path):
                remove_local_path(local_path)
        except Exception as cleanup_exc:
            logger.warning(f"Final cleanup failed for {locals().get('local_path', 'unknown')}: {cleanup_exc}")
